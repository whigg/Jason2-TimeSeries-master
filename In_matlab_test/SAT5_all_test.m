...
load('EGM2008_5.mat') % loading EGM 2008 to provide height from Geoid
...
%                 [N2,E2]=ell2utm(mat{i,2}(j,3)*pi/(180),mat{i,2}(j,2)*pi/(180));
                [N2,E2]=ell2utm(mat{i,2}(j,2)*pi/(180),mat{i,2}(j,3)*pi/(180));
                Dist=sqrt((N1-N2)^2+(E1-E2)^2);
                if Dist<=SR
%                     if length(mat{i,2}(j,:))==179
                    if length(mat{i,2}(j,:))==138
                        TmSri(pp,:)=mat{i,2}(j,:);
                    end
...
%     TmSric(:,180)= griddata(XX(:),YY(:),GH(:),lon,lat);
    TmSric(:,139)= griddata(XX(:),YY(:),GH(:),lon,lat);
end
...
%         Correction_ku=InvBar_ku+SeSbias_ku+IonCor_ku+OcTide_ku+PoTide_ku+ETide_ku+WTCor_ku+DTCor_ku+TmSric(i,180);
        Correction_ku=InvBar_ku+SeSbias_ku+IonCor_ku+OcTide_ku+PoTide_ku+ETide_ku+WTCor_ku+DTCor_ku+TmSric(i,139);
    ...
    for i=2:m
        ...
        d(i,1)=str2num(a(7:8));
        if d(i,1)~=d(i-1,1) | i==m % 'i' is 't'
            s=s+1; 
            ind(s,1)=i; % 'i' start from 1 
            TS(s-1,1)=y(i-1,1);
            TS(s-1,2)=mo(i-1,1);
            TS(s-1,3)=d(i-1,1);
